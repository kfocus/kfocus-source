...
_EOT
  ...
  _dbusSendExeFn () {
    echo "REQUEST dbus-send $*" >>"${_runFile};"
  }
  _upowerExeFn () {
  ...
  _cm2DbusSendExe='_dbusSendExeFn';
  _upowerExe='_upowerExeFn';

  # shellcheck source=../../../package-power-bin/usr/lib/kfocus/bin/kfocus-pstate
  source "${_t00TopDir}/package-power-bin/usr/lib/kfocus/bin/kfocus-pstate";
  1>&2 echo "${_t00TopDir}/package-power-bin/usr/lib/kfocus/bin/kfocus-pstate";
}
## . END _setMocksFn }
...
_runTestFn () {
  declare _cr _fail_count _assert_table _assert_count _assert_idx \
    ...
    _cmd_list _msg _check_str _count_str _wrap_str;
  ...
  for _assert_line in "${_assert_table[@]}"; do
    ...
    for _lspci_str in 'nvidia' 'intel'; do
      ...
      for _prime_select_str in 'intel' 'nvidia' 'on-demand'; do
        ...
        # Hack: Trim off leading path from runUnitTests for repeatability
        _wrap_str="${_wrap_str/#*runUnitTests:/.\/runUnitTests:}";
        ...
        # Normalize output
        sed -i 's?^warn: Called without arguments from: /bin/bash.*$?warn: Called without arguments from: /bin/bash?g' "${_runFile}";
