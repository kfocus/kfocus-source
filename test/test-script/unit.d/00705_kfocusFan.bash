...
_overwriteWithMocksFn () {
  ...
  _exe_file+='/package-power-bin/usr/lib/kfocus/bin/kfocus-fan-set';
  ...
  for _assert_line in "${_assert_table[@]}"; do
    for _inner_str in 'installed' 'NOT INSTALLED'; do
      ...
      if [ "${_inner_str}" = 'installed' ]; then
        ...
        # This is a kfocus-fan-set variable for a character device.
        #   Setting it to /dev/null imitates having it in place.
        # bashsupport disable=BP2001
        _tuxedoIoFile='/dev/null';
      else
        # This is a kfocus-fan-set variable for a character device.
        #   Setting to /bin/false imitates having no device file.
        _tuxedoIoFile='/bin/false';
