...
_echoFormatFn   () { printf '  %-4.4s : %-14.14s %-2.2s %-14.14s ' "$@"; }
...
#
# STRINGS AND LISTS
#   _cm2Arg01 (arg1)  : First  argument (DEPRECATED)
#   _cm2Arg02 (arg2)  : Second argument (DEPRECATED)
#
#   _cm2ModelCode     : Cached model code for system like 'm2g6'
#   _cm2ModelLine     : Matching ModelTable line like 'm2g6|m2g6|M2 GEN 6|...'
#   _cm2ModelHeadList : Header fields like [ code config_code label ... ]
#   _cm2ModelTable    : Table of supported models
#
#   _cm2KdiagExe      : Kdialog command string; '' if not found
#   _cm2IsTty         : 'y' or 'n' if this is a tty session
#
#   _cm2BlockMsg      : Message for _cm2ErrFn, _cm2SucFn, and _cm2WarnFn
#   _cm2CliType       : cli type, (cli|whiptail)
#   _cm2DmiProductNameFile : DMI Product name file
#   _cm2DmiProductSkuFile  : DMI SKU file
#
...
#   _cm2LogExe        : Logger command; '' if not found
#   _cm2LogLevelIdx   : Current log level (5 is default, see syslog)
...
#
# KEY
#   <key>[_][A] <methodName> : <description>
#   key            = key for test
#   [_]            = if present, see the key below for additional docs
#   [A]            = if present, see the key below for Tests
...
#   04a  _cm2ErrStrFn    : stderr "error: $input_str"
...
#   0a1  _cm2LsFn        : Cleanly list files with Find
#
...
#   0k_  _cm2CalcModelCodeFn     : If unset, calc and set _cm2ModelCode
#   0l_  _cm2CalcModelLineFn     : If unset, calc and set _cm2ModelLine
#   0m_  _cm2MakeModelHeadListFn : Creates list column titles from model table header
#   0n_A _cm2EchoModelStrFn      : Echo the ${_search_key} field for host system
#   0o_  _cm2ChkDupRunFn         : Check if another process is running the script
#                                : (deprecated, use flock)
#   0o0_ _cm2ReinstallPkgsFn     : Reinstall packages always, allow downgrades
#   0o1_ _cm2SmartInstallPkgsFn  : Install, reinstall, upgrade pkgs unattended
#   0p_A _cm2ChkInstalledPkgFn   : Quickly check if a package is installed
#   0q_A _cm2ChkPkgFn            : Quickly check if a package is available
...
#   0t1_ _cm2SetIsTtyFn          : Override and set _cm2IsTtty for tests
...
#   0y_A _cm2SetLogTagFn, _cm2SetLogLevel, _cm2LogFn
#
#   10   _cm2CatStripEchoFn         : Cat file with spaces and cr removed
#   11   _cm2SetCliTypeFn           : Set _cm2CliType to 'cli' or 'whiptail'
#   12   _cm2RunPlasmaScriptFn      : Run a plasmashell script
#   13   _cm2EchoGraphicalLoginInfoFn : Echo username and UID for graphical
#                                       logins
...
# Outputs   : Prints a block end message to STDERR
# Returns   : none
## . END _cm2ErrFn }
...
# Returns   : 0 on success, 1 if the user does not have sudo right.
## . END _cm2GetEscExeFn }
...
# Example   : _cm2AskExitFn 0 'Process is complete. Press return to exit.';
...
# Returns   : 'exit' with the provided integer
## . END _cm2AskExitFn }
...
# Returns   : 98 if the user approved the exit
## . END _cm2InterruptFn }
...
# Summary   : _cm2RunLongCmdFn [AS_USER] <_prompt_msg> <_command>
#             [ <arg1>, ... ]
# Purpose   : Run command with spinner in GUI or TTY
# Example   : _cm2RunLongCmdFn 'Install packages' 'apt' 'install' "${list[@]}";
# Arguments : $1 = _prompt_msg, $2 = _command, $3... command arguments
#    By default, commands run as root via sudo or pkexec. The special string
#    'AS_USER' can be placed before all other arguments to run the command as
#    a standard user.
...
# Purpose   : Calc system model code and set _cm2ModelCode
...
# Purpose   : Echo the ${_search_key} field for host system
# Example   : _cm2EchoModelStrFn 'is_nv_sys'
# Arguments : $1 = field_key
#           : See header in _cm2ModelTable for a list of fields.
...
_audit_cm2EchoModelStrFn () {
  ...
  declare _assert_table _assert_count _assert_idx _fail_count \
    _loop_line _col_idx _col_key _did_assert_fail _bit_list _name_str \
    _expect_str _calc_str _restore_name_file _restore_sku_file;
  ...
  _assert_table=(
    ...
    'NUC12WSHi22|nxg2|n|nxg1'
    'NUC12WSHi|nxg2|n|nxg1'
    ...
    'iridium|ir14g3|n|ir1Xg3|LCS14-15u'
  );
  ...
  for _loop_line in "${_assert_table[@]}"; do
   ...
   # Mock model change
    ...
    # Compare
    {  printf '  %-40.40s' "Assert for ${_name_str} ";
    ...
    for _col_key in code is_nv_sys config_code; do
      ...
      _calc_str="$(_cm2EchoModelStrFn "${_col_key}")";
      if [ "${_expect_str}" = "${_calc_str}"  ]; then
        printf '  ok   : Col key | %-12.12s | matches | %-12.12s |\n' \
          "${_col_key}" "${_expect_str}";
      else
        printf '  fail : Col key | %-12.12s | NOMATCH | %-12.12s |\n' \
          "${_col_key}" "${_expect_str}";
        _cm2EchoFn "    Found string is |${_calc_str}|";
        _did_assert_fail='y';
      fi
      (( _col_idx++ ));
    done
...
# 0o0_
## BEGIN _cm2ReinstallPkgsFn {
# Summary   : _cm2ReinstallPkgsFn <package-list>
# Purpose   : Reinstall all packages listed, always.
#   Retain configuration files only if changed.
## . END _cm2ReinstallPkgsFn }

# 0o1_
## BEGIN _cm2SmartInstallPkgsFn {
# Summary   : _cm2SmartInstallPkgsFn <package-list>
# Purpose   : Install, reinstall, upgrade, downgrade unattended.
#   For each package in <package-list>:
#     * SAME: Install if not currently installed, mark manual
#     * SAME: If installed, only reinstall IFF there is an upgrade
#             or downgrade available.
#     * SAME: Will remove broken dependency packages if required
#     * SAME: Work with downgrades forced by pinning.
...
#     * SAME: Does not attempt interactive control
#         (same as install with the 'noninteractive' setting)
# Example   : _cm2SmartInstallPkgsFn dkms
# Arguments : "${@}" - list of package to install or upgrade
# Globals   : DEBIAN_FRONTEND
# Outputs   : Normal apt-get minimal output
# Returns   : The result of apt-get call
## . END _cm2SmartInstallPkgsFn }
...
# Purpose   : Quickly check if a package is available
...
_audit_cm2ChkPkgFn () {
  ...
  for _loop_line in "${_assert_table[@]}"; do
    IFS='|' read -r -a _bit_list <<< "${_loop_line}";
    printf '  %-40.40s' "Assert for ${_bit_list[0]}";
    ...
    printf '  %-40.40s' "Assert ${_bit_list[0]} v${_chk_str}";
...
# Purpose   : For all in the arg list, assign value to global variable <name>
...
_audit_cm2AssignExeVarsFn () {
  ...
  _assert_table=(
    ...
    # Find the second path, so returns 0.
    'snuggle_bear,/usr/bin/awk   | exe | required | 0 |/usr/bin/awk'
    'pizzaX,pizzaY,/usr/bin/tr   | exe | required | 0 |/usr/bin/tr'
    # No path works, so return 1
    'pizzaX,pizzaY,/usr/bin/XXNO | exe | required | 1 |'

    # Since cmd is not found sets as ''. Ret is 0 because optional.
    ...
    # Since the file is not rw, sets as ''. Ret is 0 because optional.
  ...
  { printf '  %-4.4s : %-17.17s|%-8.8s|%-8.8s|%-1.1s|%-17.17s\n' \
...
#   * _do_strip_last options are as follows:
#     y) If more than one line, remove the last line.
#        This is so final lines like 'Press OK to continue'
#        can be removed and replaced line like
#        'Press <return> to continue' when converted to CLI.
#        This is used with the 'p' option in cm2PromptUserFn.
#     w) Leave the string alone (without changes)
#     n) (default) Same as w, except remove trailing '?'
#
...
_audit_cm2StripTagsFn () {
  declare _input_multi_str   _expect_multi_w_str        \
    _expect_multi_n_str      _expect_multi_y_str        \
    _input_single_str        _expect_single_w_str       \
    _expect_single_n_str     _expect_single_y_str       \
    _input_multi_noq_str     _expect_multi_noq_w_str    \
    _expect_multi_noq_n_str  _expect_multi_noq_y_str    \
    _input_single_noq_str    _expect_single_noq_w_str   \
    _expect_single_noq_n_str _expect_single_noq_y_str   \
    _assert_table _loop_line _assert_count _assert_idx  \
    _fail_count _bit_list _input_str _cmd_list          \
    _expect_str _parse_key _output_str _stat_str;

  _input_multi_str='<html>
<h1>Resistance Is Futile</h1>
<p style="margin-left:30px">Or is it?<br></p>

<p style="margin-left:30px">Continue now?</p>
</html>
';
  _expect_multi_w_str='Resistance Is Futile
Or is it?

Continue now?';
  _expect_multi_n_str="$(sed '$ s|\?\s*$||g' <<< "${_expect_multi_w_str}")";
  _expect_multi_y_str="$(head -n-1 <<< "${_expect_multi_w_str}")";

  _input_single_str='<p class="brute" style="foo">Shall we continue?</p>';
  _expect_single_w_str='Shall we continue?';
  _expect_single_y_str="${_expect_single_w_str}";
  _expect_single_n_str='Shall we continue';

  _input_multi_noq_str='<html>
<h1>Join the Boating Party</h1>

<p style="margin-left:30px">Do not underestimate
all the fun?<br></p>

<p>Bring a friend? Or a buddy!</p>

<p style="margin-left:30px">Press OK to continue.</p>
</html>
';
  _expect_multi_noq_w_str='Join the Boating Party
...
Press OK to continue.';
  _expect_multi_noq_n_str="${_expect_multi_noq_w_str}";
  _expect_multi_noq_y_str="$(head -n-1 <<<"${_expect_multi_noq_w_str}")";

  _input_single_noq_str='<p class="fred">Press OK to continue</p>';
  _expect_single_noq_w_str='Press OK to continue';
  _expect_single_noq_n_str="${_expect_single_noq_w_str}";
  _expect_single_noq_y_str="${_expect_single_noq_w_str}";

  _assert_table=(
    "${_input_multi_str}|${_expect_multi_w_str}|w"
    "${_input_multi_str}|${_expect_multi_n_str}|n"
    "${_input_multi_str}|${_expect_multi_y_str}|y"
    "${_input_multi_str}|${_expect_multi_n_str}"

    "${_input_single_str}|${_expect_single_w_str}|w"
    "${_input_single_str}|${_expect_single_n_str}|n"
    "${_input_single_str}|${_expect_single_y_str}|y"
    "${_input_single_str}|${_expect_single_n_str}"

    "${_input_multi_noq_str}|${_expect_multi_noq_w_str}|w"
    "${_input_multi_noq_str}|${_expect_multi_noq_n_str}|n"
    "${_input_multi_noq_str}|${_expect_multi_noq_y_str}|y"
    "${_input_multi_noq_str}|${_expect_multi_noq_n_str}|"

    "${_input_single_noq_str}|${_expect_single_noq_w_str}|w"
    "${_input_single_noq_str}|${_expect_single_noq_n_str}|n"
    "${_input_single_noq_str}|${_expect_single_noq_y_str}|y"
    "${_input_single_noq_str}|${_expect_single_noq_n_str}"
  );
  ...
  _fail_count=0;
  for _loop_line in "${_assert_table[@]}"; do
    readarray -td '|' _bit_list < <( echo -n "${_loop_line}" );
    _input_str="${_bit_list[0]}";
    _expect_str="${_bit_list[1]}";
    _cmd_list=('_cm2StripTagsFn' "${_input_str}");
    _parse_key="${_bit_list[2]:-}";
    if [ -n "${_parse_key}" ]; then _cmd_list+=("${_parse_key}"); fi
    _output_str="$("${_cmd_list[@]}")";

    if [ "${_output_str}" = "${_expect_str}" ]; then
      _stat_str='ok'; else _stat_str='fail';
    fi
    {  printf '  %-4.4s : %-.02s %-23.23s %-2.2s %-23.23s %02i / %02i \n' \
      "${_stat_str}" "${_parse_key:- }" \
      "${_output_str}" '==' "${_expect_str}" \
      "${_assert_idx}" "${_assert_count}";
    } >&2

    if [ "${_stat_str}" = 'fail' ]; then (( _fail_count++ )); fi
    (( _assert_idx++ ));
  done
...
#           :   'A' yesno prompt; default answer is 'n'
...
# Returns   : none
# TODO      : Document new modes and whiptail functionality
...
_audit_cm2PromptUserFn () {
  ...
  # TODO: Test whiptail mode!

  _assert_table=( 'a|y' 'a|n' 'A|n' 'A|n' 'w|y' 'w|n' 'p|' );
  _exit_int=0;
  ...
  for _is_tty in n y; do
    ...
    for _loop_line in "${_assert_table[@]}"; do
      ...
      if [ "${_is_tty}" = 'y' ]; then
        ...
        _prompt_str="Confirm the query and title |${_type_str}|. <br>";
      fi
...
_audit_cm2TestVersFn () {
  declare _assert_table _loop_line _assert_count _assert_idx _fail_count \
    _bit_list _v1_str _v2_str _chk_str _stat_str _op_str;
  ...
  for _loop_line in "${_assert_table[@]}"; do
    ...
    if [ "${_chk_str}" = 'true' ]; then
      ...
      _op_str='<=';
      if _cm2TestVersFn "${_v1_str}" "${_v2_str}"; then
        _stat_str='fail'; else _stat_str='ok';
      fi
    fi
    {  _echoFormatFn "${_stat_str}" "${_v1_str}" "${_op_str}" "${_v2_str}";
       printf '%02i / %02i \n' "${_assert_idx}" "${_assert_count}";
    } >&2
...
## . END _audit_cm2TestVersFn }

# 0y_A _cm2SetLogTagFn, _cm2SetLogLevel, _cm2LogFn
# Summary   : _cm2SetLogTagFn <tag_str>
# Purpose   : Set a prefix for log messages sent to syslog. Must be set.
# Example   : _cm2SetLogTagFn "${_baseName}";
# Arguments : tag_str (required)
# Globals   : _cm2LogTag is set to value provided
# Outputs   : none
# Returns   : 0 success, 1 failure

# Summary   : _cm2SetLogLevelFn <priority_key>
# Purpose   : Set the low-pass filter for logging, _cm2LogLevelIdx.
# Example   : _cm2SetLogLevelFn 'notice';
# Arguments : priority_key
# Globals   : _cm2LogLevelIdx is set to value from _cm2LogIdxMap
# Outputs   : none
# Returns   : 0 success, 1 failure
...
declare _cm2LogLevelIdx _cm2LogTag; # Hack to get around shellcheck
## BEGIN _audit_cm2SyslogFn {
_audit_cm2SyslogFn () {
  declare _assert_table _loop_line _assert_count _assert_idx _fail_count \
    _bit_list _cmd_str _a1_str _a2_str _a3_str _stat_str \
    _calc_str _expect_str;

  _assert_table=(
   ' log   |   debug | verbose    |DNR'
   ' log   |  notice | masses     |DNR'
   ' log   |    warn | cautious   |DNR'
   ' log   |     err | unexpected |DNR'
   ' log   |    crit | The worst  |DNR'
   ' sett  | AA      | AA[X]'
   ' level | debug   | 7'
   ' log   |   debug | verbose    |AA[X] - DEBUG: verbose '
   ' log   |  notice | masses     |AA[X] - NOTICE: masses '
   ' log   |    warn | cautious   |AA[X] - WARN: cautious '
   ' log   |     err | unexpected |AA[X] - ERR: unexpected'
   ' log   |    crit | The worst  |AA[X] - CRIT: The worst'
   ' level | warn    | 4'
   ' log   |   debug | verbose    |'
   ' log   |  notice | masses     |'
   ' log   |    warn | cautious   |AA[X] - WARN: cautious '
   ...
   ' log   |  notice | masses     |'
   ' log   |    warn | cautious   |'
   ' log   |     err | unexpected |BB[X] - ERR: unexpected'
   ' log   |    crit | The worst  |BB[X] - CRIT: The worst'
  );
  _assert_count="${#_assert_table[@]}";
  _assert_idx=1;
  _fail_count=0;
  ...
  for _loop_line in "${_assert_table[@]}"; do
    IFS='|' read -r -a _bit_list <<< "${_loop_line}";
    _cmd_str="$( _echoScrubStrFn "${_bit_list[0]}")";
    _a1_str="$( _echoScrubStrFn "${_bit_list[1]}")";
    _a2_str="$(_echoScrubStrFn "${_bit_list[2]:-}")";
    _a3_str="$(_echoScrubStrFn "${_bit_list[3]:-}")";

    case "${_cmd_str}" in
      log)
        _expect_str="${_a3_str}";
        if _calc_str="$(_cm2LogFn "${_a1_str}" "${_a2_str}" 2>&1)"; then
          # shellcheck disable=SC2001
          _calc_str="$(sed 's/[0-9]\+/X/g' <<< "${_calc_str}")";
        else
          _calc_str="DNR";
        fi
        ;;
      level)
        if _cm2SetLogLevelFn "${_a1_str}"; then
          _calc_str="${_cm2LogLevelIdx}";
          _expect_str="${_a2_str}";
        fi
        ;;
      sett)
        if _cm2SetLogTagFn "${_a1_str}"; then
          # shellcheck disable=SC2001
          _calc_str="$(sed 's/[0-9]\+/X/g' <<< "${_cm2LogTag}")";
          _expect_str="${_a2_str}";
        else
          # shellcheck disable=SC2001
          _calc_str="$(sed 's/[0-9]\+/X/g' <<< "${_cm2LogTag}")";
          _expect_str="${_calc_str}";
        fi
        ;;
      *)
        _cmd_str='Bad';
        _calc_str='Bad command:';
        _expect_str="${_cmd_str}";
        ;;
    esac

    _stat_str='fail';
    [ "${_calc_str}" = "${_expect_str}" ] && _stat_str='ok';

    {  printf '  %-4.4s : %-6.6s %-30.30s %-30.30s ' \
      "${_stat_str}" "${_cmd_str}" "${_calc_str}" "${_expect_str}";
       printf '%02i / %02i \n' "${_assert_idx}" "${_assert_count}";
    } >&2
...
_runTestFn () {
  ...
  _audit_fn_list=(
    ...
    '_audit_cm2EchoModelStrFn'
    '_audit_cm2ChkInstalledPkgFn'
    '_audit_cm2ChkPkgFn' # Includes cm2ChkInstalledVersFn
    '_audit_cm2PromptUserFn'
    '_audit_cm2TestVersFn'
    '_audit_cm2SyslogFn'
  );
