...
_echoCpuModelNameFn () {
  ...
  _model_line="$(lscpu | grep -Ei '^\s*model name' | cut -f2 -d: |sed 's/^\s*//' )";

  case "${_model_line}" in
    'Intel(R) Core(TM) Ultra 5 225H' ) _cpu_name='u5-225h';;
  ...
  #
  if [ "${_model_line}" = "${_cpu_name}" ] || [ -z "${_cpu_name}" ]; then
    _cpu_name='unknown';
  fi
...
_runTestFn () {
  ...
  _cm2EchoFn 'Read existing power mode';
  _current_mode_str="$(sudo "${_powerSetExe}" -r)";

  # As of kernel 6.11, this is important as
  #   lower profiles will result in non-turbo max cpu freqs.
  _cm2EchoFn 'Reset to Normal frequency profile';
  sudo "${_powerSetExe}" "Normal";
  sleep 1;
  ...
  _runAssertsFn "${_pick_list[@]:1}"; # Remove header
  _fail_count="$?";
  _total_fail_count=$((_total_fail_count+_fail_count))
  _assert_count="$(echo "${#_pick_list[@]}*2"|bc)";
  (( _assert_count -= 2 )) || true; # Adjust for header

  if [ "${_fail_count}" -gt 0 ]; then
    _cm2EchoFn "FAIL SUITE 2: ${_fail_count} / ${_assert_count} failed.\n";
  else
    _cm2EchoFn 'OK   SUITE 2\n';
  fi
  ...
  _runAssertsFn "${_pick_list[@]:2}"; # Remove headers
  _fail_count="$?";
  _total_fail_count=$((_total_fail_count+_fail_count))
  _assert_count="$(echo "${#_pick_list[@]}*2"|bc)";
  (( _assert_count -= 4 )) || true; # Adjust for headers
